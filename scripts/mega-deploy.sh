...
# Initialize kubeadm
kubeadm init --token "$TOKEN" --api-port 443 || rm -f /var/lib/kubelet/* && kubeadm init --token "$TOKEN" --api-port 443 --skip-preflight-checks
...
install_node_ubuntu_centos() {
# Join cluster
kubeadm join --api-port 443 --token $TOKEN $MASTER || rm -f /var/lib/kubelet/* && kubeadm join --api-port 443 --token $TOKEN $MASTER --skip-preflight-checks
}
